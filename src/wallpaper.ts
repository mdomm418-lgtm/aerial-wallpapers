...
export const LiveWallpaper = GObject.registerClass(
    class LiveWallpaper extends St.Widget {
        ...
        private getRenderer(): Clutter.Actor | null {
            ...
            const rendererWindowActors = this.getWindowActors().filter(
                actor => actor.meta_window?.title?.includes(APPLICATION_ID)
            );

            const numMonitors = global.display.get_n_monitors();
            if (rendererWindowActors.length < numMonitors) {
                logger.debug(
                    `Aerial windows (${rendererWindowActors.length}) < monitors (${numMonitors}), rejecting`
                );
                return null;
            }

            const monitorIndices = rendererWindowActors.map(actor => actor.meta_window!.get_monitor());
            ...
            const renderer = rendererWindowActors.find(
        ...
        private fade(visible = true, duration = BACKGROUND_FADE_ANIMATION_TIME): void {
            if (this.isDisposed)
                return;
        ...
        /**
         * When the renderer cannot fetch video, fade out so the static GNOME
         * desktop picture shows instead of a black void.
         *
         * @param {boolean} active - whether the fallback desktop should show
         */
        setDesktopFallback(active: boolean): void {
            if (this.isDisposed)
                return;
            this.fade(!active, active ? 500 : 900);
        }
    }
);
