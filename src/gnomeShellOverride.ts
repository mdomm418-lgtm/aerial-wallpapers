...
export class GnomeShellOverride {
    ...
    private renderer: RendererWrapper | null = null;
    private desktopFallbackSignalId: number | null = null;
    ...
    enable(): void {
        ...
        this.desktopFallbackSignalId = this.renderer.proxy.connectSignal(
            'desktopFallbackChanged',
            (_proxy: Gio.DBusProxy, _sender: string, [active]: [boolean]) => {
                for (const actor of this.wallpaperActors)
                    actor.setDesktopFallback(active);
            }
        );
    ...
    disable(): void {
        ...
        if (this.desktopFallbackSignalId !== null && this.renderer) {
            this.renderer.proxy.disconnectSignal(this.desktopFallbackSignalId);
            this.desktopFallbackSignalId = null;
        }
        this.renderer = null;
